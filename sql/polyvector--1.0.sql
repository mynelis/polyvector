...
--
-- NOTE:
--   Dimensions 3/4/5/6 are stand-ins for 384/768/1024/1536
--   to make testing easy. Replace with real dimensions in your
--   fork or future versions if desired.
-- ============================================================
...
BEGIN
  ...
  IF e.dims IS NULL OR e.dims NOT IN (3, 4, 5, 6) THEN
    RETURN false;
  END IF;
  ...
  IF e.dims = 3 THEN RETURN e.v384  IS NOT NULL AND e.slot = 'v384';  END IF;
  IF e.dims = 4 THEN RETURN e.v768  IS NOT NULL AND e.slot = 'v768';  END IF;
  IF e.dims = 5 THEN RETURN e.v1024 IS NOT NULL AND e.slot = 'v1024'; END IF;
  IF e.dims = 6 THEN RETURN e.v1536 IS NOT NULL AND e.slot = 'v1536'; END IF;
...
BEGIN
  ...
  IF e.dims = 3 THEN RETURN e.v384;
  ELSIF e.dims = 4 THEN RETURN e.v768;
  ELSIF e.dims = 5 THEN RETURN e.v1024;
  ELSIF e.dims = 6 THEN RETURN e.v1536;
...
BEGIN
  ...
  IF d = 3 THEN slot_field := 'v384';  slot_label := poly_col || '.v384';
  ELSIF d = 4 THEN slot_field := 'v768';  slot_label := poly_col || '.v768';
  ELSIF d = 5 THEN slot_field := 'v1024'; slot_label := poly_col || '.v1024';
  ELSIF d = 6 THEN slot_field := 'v1536'; slot_label := poly_col || '.v1536';
  ELSE
    RAISE EXCEPTION 'Unsupported dims: %', d;
  END IF;
